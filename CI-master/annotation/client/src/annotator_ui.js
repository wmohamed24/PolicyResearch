        ...
        if (spanTypes[originType]) {
          ...
          $.each(arcTypes || [], function(arcTypeNo, arcDesc) {
            if (arcDesc.targets && arcDesc.targets.indexOf(targetType) != -1) {
              ...
              var $label = $('<label class="arc_type_label ui-corner-all" for="arc_' + arcTypeName + '"/>').text(displayName);
              var $div = $('<div class="arc_content"/>').append($checkbox).append($label);
              $scroller.append($div);
              if (arcDesc.hotkey) {
        ...
        if (arcDragOrigin) {
          ...
          if (chunkIndexFrom !== undefined && chunkIndexTo !== undefined) {
            ...
            while (selectedFrom < selectedTo && " \n\t".indexOf(data.text.substr(selectedTo - 1, 1)) !== -1) selectedTo--;
              ...
              //Yan: addded this to make sure whole words are selected
              while (data.text.substr(selectedFrom,1).search("\'|^[a-zA-Z0-9_]*$") !== -1) selectedFrom--;
              while (data.text.substr(selectedTo,1).search("\'|^[a-zA-Z0-9_]*$") !== -1) selectedTo++;
              selectedFrom +=1;
...
*/
      ...
      var addSpanTypesToDivInner = function($parent, types, category) {
        ...
        $.each(types, function(typeNo, type) {
          if (type === null) {
            ...
            spanBgColor = Util.adjustColorLightness(spanBgColor, spanBoxTextBgColorLighten);
            var $label = $('<label class="span_type_label ui-corner-all"/>').
      ...
      $('#undo_button').click(undoFunction);
